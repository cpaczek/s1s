...
export const mapCommand = defineCommand({
  ...
  args: {
    ...treeArgs,
    subject: { type: "positional", required: true, description: "Subject to map" },
    scope: { type: "string", default: "", description: "Directory scope" },
    json: { type: "boolean", default: false, description: "Print SearchResult JSON" },
  },
  async run({ args }) {
    const result = await map(loadTree(args), createClient(), args.subject, {
      scope: args.scope,
