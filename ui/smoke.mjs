...
const page = await browser.newPage({ viewport: { width: 1440, height: 960 }, colorScheme: "light", isMobile: true, hasTouch: true });
...
try {
  ...
  await page.addInitScript(() => sessionStorage.setItem("s1s.walkthrough-seen.v1", "1"));
  ...
  await page.waitForFunction(() => Number(document.querySelector('#elapsedMs').textContent.replaceAll(',', '')) > 60);
  await page.locator('#stopSearch').click(); delay = false;
  const cancelledTime = await page.locator('#elapsedMs').textContent();
  await page.waitForTimeout(100);
  assert.equal(await page.locator('#elapsedMs').textContent(), cancelledTime, 'Cancelled timer freezes');
  ...
  // Keep the form anchored, bring real activity into view, and replay without HTTP.
  await page.setViewportSize({width:390,height:844});
  await page.goto(base); await page.waitForFunction(() => !document.querySelector('#question').disabled);
  const documentTop = selector => page.locator(selector).evaluate(e => e.getBoundingClientRect().top + scrollY);
  const formTop = await documentTop('#searchForm');
  delay = true;
  await page.locator('#question').fill('Where is configuration?');
  await page.locator('#question').press('Enter');
  await page.waitForFunction(() => document.querySelector('#activity').getBoundingClientRect().top >= 0 && document.querySelector('#activity').getBoundingClientRect().top < 30);
  assert.equal(await page.locator('#question').evaluate(e => e === document.activeElement), false, 'Dismiss mobile keyboard');
  assert(Math.abs(await documentTop('#searchForm') - formTop) < 1, 'Submission must not collapse content above the animation');
  const marksTop = await documentTop('#fileActivity');
  const visibleMarks = await page.locator('#fileActivity').boundingBox();
  assert(visibleMarks.y >= 0 && visibleMarks.y + visibleMarks.height < 844, 'Animation visible on submit');
  await page.locator('#stopSearch').click(); delay = false;
  await page.locator('#retrySearch').click();
  await page.waitForFunction(() => !document.querySelector('#searchResults').hidden);
  assert(Math.abs(await documentTop('#fileActivity') - marksTop) < 1, 'Completion must not collapse the animation');
  await page.waitForTimeout(150);
  const completedMarks = await page.locator('#fileActivity').boundingBox();
  assert(completedMarks.y >= 0 && completedMarks.y + completedMarks.height < 844, 'Browser scroll anchoring must not follow repo notes below newly inserted results');
  const callsBeforeReplay = requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length;
  const resultBeforeReplay = await page.locator('#fileResults').textContent();
  await page.locator('#replaySearch').evaluate(e => { e.click(); document.querySelector('#pauseReplay').click(); });
  assert.match(await page.locator('#activityLabel').textContent(), /Recorded replay/);
  assert.equal(await page.locator('.activity-file').count(), 0, 'Replay starts at the recorded queue, not fabricated activity');
  await page.locator('#nextReplay').click();
  assert.equal(await page.locator('.activity-file').count(), paths.length);
  assert(Math.abs(await documentTop('#fileActivity') - marksTop) < 1, 'Replay keeps animation position');
  const pausedStep = await page.locator('#replayPosition').textContent();
  await page.waitForTimeout(1700);
  assert.equal(await page.locator('#replayPosition').textContent(), pausedStep, 'Manual next stays paused');
  assert.equal(await page.locator('#replaySpeed').count(), 0, 'Playback has no speed selector');
  const quickStarted = Date.now();
  await page.locator('#pauseReplay').click();
  await page.waitForFunction(() => document.querySelector('#activityTitle').textContent === 'Walkthrough complete');
  assert(Date.now() - quickStarted < 2500, 'Default replay is very quick');
  assert.equal(await page.locator('#fileResults').textContent(), resultBeforeReplay, 'Replay preserves original results');
  assert.equal(requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length, callsBeforeReplay, 'Replay makes no new inference calls');
  await page.screenshot({path:join(root,'.shots/mobile-replay.png'),fullPage:true});
  await page.locator('#replaySearch').click();
  await page.locator('#repository').selectOption('outline');
  await page.waitForTimeout(800);
  assert.equal(await page.locator('#activity').isVisible(), false, 'Changing repo cancels playback');
  // A fresh tab gets one paced walkthrough, while elapsed time remains the real request.
  const introPage = await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});
  introPage.on('pageerror', e => errors.push(e.message));
  await introPage.goto(base);
  await introPage.waitForFunction(() => !document.querySelector('#question').disabled);
  await introPage.locator('#question').fill('Where is configuration?');
  const introCalls = requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length;
  await introPage.locator('#question').press('Enter');
  await introPage.waitForFunction(() => document.querySelector('#activityLabel').textContent.includes('First search walkthrough'));
  assert(await introPage.locator('#searchResults').isVisible(), 'Original answer remains accessible during the introduction');
  const realTime = await introPage.locator('#elapsedMs').textContent();
  assert(Number(realTime.replaceAll(',', '')) >= 300, 'Counter measures actual request time');
  await introPage.waitForTimeout(400);
  assert.equal(await introPage.locator('#elapsedMs').textContent(), realTime, 'Walkthrough cannot inflate actual search time');
  await introPage.screenshot({path:join(root,'.shots/first-walkthrough.png'),fullPage:true});
  await introPage.locator('#skipReplay').click();
  assert.equal(await introPage.locator('#activityTitle').textContent(), 'Walkthrough complete');
  assert.equal(requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length, introCalls + 1, 'Automatic walkthrough makes no extra request');
  await introPage.locator('#question').press('Enter');
  await introPage.waitForFunction(() => document.querySelector('#activityLabel').textContent === 'Completed search');
  assert(await introPage.locator('#skipReplay').isHidden(), 'Only the first completed search auto-plays');
