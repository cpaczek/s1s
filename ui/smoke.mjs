...
try {
  ...
  const callsBeforeSlowReplay = requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length;
  ...
  await page.locator('#replaySpeed').selectOption('700');
  await page.locator('#pauseReplay').click();
  await page.waitForFunction(() => document.querySelector('#activityTitle').textContent === 'Replay complete');
  assert.equal(await page.locator('#fileResults').textContent(), resultBeforeReplay, 'Replay preserves original results');
  assert.equal(requests.filter(u => ['/api/search','/api/explain'].includes(u.pathname)).length, callsBeforeSlowReplay, 'Slow replay makes no new inference calls');
