...
async function execute(request: Request, env: DemoEnv, ctx: ExecutionContext, url: URL, repo: Repository): Promise<Response> {
  ...
  const coordinator = env.ADMISSION.getByName("paid-demo-v1");
  ...
  async fetch(request: Request, env: DemoEnv, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/")) {
        ...
        if (url.pathname === "/api/search" || url.pathname === "/api/explain") return await execute(request, env, ctx, url, repo);
